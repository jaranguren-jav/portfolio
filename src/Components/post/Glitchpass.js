...
GlitchPass.prototype = Object.assign(Object.create(Pass.prototype), {
  ...
  render: function(renderer, writeBuffer, readBuffer, deltaTime, maskActive) {
    ...
    const glitchIntensity = this.glitchIntensity
    this.uniforms['tDiffuse'].value = readBuffer.texture
    this.uniforms['seed'].value = Math.random() * factor //default seeding
    this.uniforms['byp'].value = 0
    if (factor && glitchIntensity) {
      this.uniforms['amount'].value = (Math.random() / 90) * factor
      this.uniforms['angle'].value = _Math.randFloat(-Math.PI, Math.PI) * factor
      this.uniforms['distortion_x'].value = _Math.randFloat(0, 0) * factor
      this.uniforms['distortion_y'].value = _Math.randFloat(0, 0) * factor
      this.uniforms['seed_x'].value = _Math.randFloat(0, glitchIntensity) * factor
      this.uniforms['seed_y'].value = _Math.randFloat(0, glitchIntensity) * factor
